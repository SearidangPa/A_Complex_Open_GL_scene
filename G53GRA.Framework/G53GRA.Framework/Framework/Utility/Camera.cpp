...
void Camera::Reset(){
	// set the camera position to start at (0, 20, middle of the window)
	eyePosition[0] = 0.0f;
	eyePosition[1] = 20.0f;
	eyePosition[2] = 4.55f * static_cast<float>(Scene::GetWindowHeight()) / static_cast<float>(tan(M_PI / 6.0));
	...
	// set the right vector to point along the -z axis
    right[0] = 0.0f;
    right[1] = 0.0f;
    right[2] = -1.0f;
...
void Camera::positionCamera() {
    ...
    eyePosition[0] = -1500+camrad * sin(camangle);                  // set eye x (at camrad*sin(0)[ = 0])
    eyePosition[1] = 1000;
    eyePosition[2] = -2000+camrad * cos(camangle);                  // set eye z (at camrad*cos(0)[ = 1])
}


void Camera::cameraRadius() {
    camrad = 2000;
}

void Camera::HandleKey(unsigned char key, int state, int x1, int y1)
{
